...
def get_nyu_data(batch_size, nyu_data_zipfile='/content/DenseDepth/nyu_data.zip'):
...
class NYU_BasicAugmentRGBSequence(Sequence):
    ...
    def __getitem__(self, idx, is_apply_policy=True):
        ...
        for i in range(batch_x.shape[0]):
            ...
            x = np.clip(np.asarray(Image.open( "/content/DenseDepth/"+sample[0] )).reshape(480,640,3)/255,0,1)
            y = np.clip(np.asarray(Image.open( "/content/DenseDepth/"+sample[1] )).reshape(480,640,1)/255*self.maxDepth,0,self.maxDepth)
